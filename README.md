...
1. generate postgres database schema and triggers by [BIRDS (bi-directional transformation library)](https://github.com/dangtv/BIRDS), and set the output files in peer/lib/dejima_sql. 
2. add files defining base tables into peer/db/migrate. Migration files in this place are called when docker container starts. For example:

```
class Create<peer_name>Users < ActiveRecord::Migration[5.2]
  def change
...
```
3. add files defining triggers into peer/db/migrate.  These files call the SQL files generated by BIRDS. For example:

```
class Create<peer_name>DejimaTable < ActiveRecord::Migration[5.2]
  def up
    return unless Rails.application.config.dejima_peer_type == :<peer_name>
    execute File.read(Rails.root / "lib" / "dejima_sql" / "<sql file generated by step 1>")
  end
...
```
4. add files defining <peer_name> model into peer/app/models. For example:
```
require 'set'

class <peer_name>User < ApplicationRecord
  include DejimaBase
end
```
Instead following procedure so far, you can also use rails command, for example: `rails generate model`. But in that case, you need to modify mannually the files generated by the command after all, so following this 4 steps is recommended.

5. add services to the [crane.yml](crane.yml). client, peer and postgres containers are required for each peer. Example of one service is below. Please replace <peer_name> with peer name. When you add new peer, you need to create all these three types of containers.
...
6. add base "table - dejima table - peer" relationship to [peer/dejima_setting.yml](peer/dejima_setting.yml). The example when you add one peer <peer_name> to Peer0 and Peer1 is below.
