...
p_0(DEJIMA_ID, VID, LOCATION, RID) :- COL4 = 'True' , bt(DEJIMA_ID, VID, LOCATION, RID, COL4).
*/
...
BEGIN
    ...
    END IF;
    DELETE FROM __temp__Δ_del_bt_for_a2 WHERE ROW(DEJIMA_ID,VID,LOCATION,RID,SHARABLE) = NEW;
    INSERT INTO __temp__Δ_ins_bt_for_a2 SELECT (NEW).*; 
    ELSIF TG_OP = 'UPDATE' THEN
    IF (SELECT count(*) FILTER (WHERE j.value = jsonb 'null') FROM  jsonb_each(to_jsonb(NEW)) j) > 0 THEN 
        RAISE check_violation USING MESSAGE = 'Invalid update: null value is not accepted';
    END IF;
    DELETE FROM __temp__Δ_ins_bt_for_a2 WHERE ROW(DEJIMA_ID,VID,LOCATION,RID,SHARABLE) = OLD;
...
FROM (SELECT __temp__Δ_ins_bt_for_a2_a5_0.SHARABLE AS COL0, __temp__Δ_ins_bt_for_a2_a5_0.DEJIMA_ID AS COL1, __temp__Δ_ins_bt_for_a2_a5_0.LOCATION AS COL2, __temp__Δ_ins_bt_for_a2_a5_0.RID AS COL3, __temp__Δ_ins_bt_for_a2_a5_0.VID AS COL4 
FROM __temp__Δ_ins_bt_for_a2 AS __temp__Δ_ins_bt_for_a2_a5_0   UNION SELECT __temp__bt_for_a2_a5_0.SHARABLE AS COL0, __temp__bt_for_a2_a5_0.DEJIMA_ID AS COL1, __temp__bt_for_a2_a5_0.LOCATION AS COL2, __temp__bt_for_a2_a5_0.RID AS COL3, __temp__bt_for_a2_a5_0.VID AS COL4 
FROM __temp__bt_for_a2 AS __temp__bt_for_a2_a5_0 
...
FROM (SELECT bt_a5_0.DEJIMA_ID AS COL0, bt_a5_0.VID AS COL1, bt_a5_0.LOCATION AS COL2, bt_a5_0.RID AS COL3, bt_a5_0.SHARABLE AS COL4 
FROM public.bt AS bt_a5_0 
...
WHERE bt_a5.SHARABLE = 'True' AND bt_a5.RID = new_a2_a4_0.COL3 AND bt_a5.LOCATION = new_a2_a4_0.COL2 AND bt_a5.VID = new_a2_a4_0.COL1 AND bt_a5.DEJIMA_ID = new_a2_a4_0.COL0 ) ) AS Δ_ins_bt_a5_0  ) AS Δ_ins_bt_extra_alia 
            EXCEPT 
            SELECT * FROM  public.bt; 

FOR temprecΔ_del_bt IN ( SELECT * FROM Δ_del_bt) LOOP 
            DELETE FROM public.bt WHERE ROW(DEJIMA_ID,VID,LOCATION,RID,SHARABLE) =  temprecΔ_del_bt;
            END LOOP;
DROP TABLE Δ_del_bt;
